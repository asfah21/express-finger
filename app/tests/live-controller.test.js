function isDuplicateWithinWindow(row, fid, type, now, windowMs = 300_000) {
...
test('live attendance duplicate rule blocks the same fid and type within five minutes', () => {
    const row = { user_id: '12', type: 0, timestamp: '2026-08-07T08:00:00+08:00' }
    expect(isDuplicateWithinWindow(row, '12', 0, new Date('2026-08-07T08:04:59+08:00').getTime())).toBe(true)
})

test('live attendance duplicate rule allows the same type after five minutes', () => {
    const row = { user_id: '12', type: 0, timestamp: '2026-08-07T08:00:00+08:00' }
    expect(isDuplicateWithinWindow(row, '12', 0, new Date('2026-08-07T08:05:00+08:00').getTime())).toBe(false)
})
