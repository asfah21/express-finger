...
export async function ensureSchema() {
  ...
  await pool.query(`
    ...
    -- Kiosk attendance devices (role 'public') must be registered and approved
    -- by a Super Admin before they can log in / record attendance. Each approved
    -- device is bound to exactly one 'public' account (1 device = 1 user).
    CREATE TABLE IF NOT EXISTS kiosk_devices (
      id BIGSERIAL PRIMARY KEY,
      device_id TEXT NOT NULL UNIQUE,          -- UUID generated by the kiosk client (persisted in localStorage)
      name TEXT DEFAULT '',                     -- human label, e.g. "Kiosk Lobby"
      status TEXT NOT NULL DEFAULT 'pending',   -- pending | approved | revoked
      user_id INTEGER REFERENCES users(id) ON DELETE SET NULL, -- bound public account
      approved_by TEXT,                         -- superadmin username who approved
      approved_at TIMESTAMPTZ,
      revoked_at TIMESTAMPTZ,
      last_seen TIMESTAMPTZ,                    -- last heartbeat / activity
    ...
    -- device_id on sessions lets us enforce "1 user = 1 device" globally and
    -- revoke all sessions of a device when it is revoked by an admin.
  ...
  if (permCount === 0) {
    const defaultPermissions = [
      ...
      { page_id: 'kiosk-devices', page_label: 'Kiosk Devices', roles: '{superadmin}' },
  ...
  // Ensure Kiosk Devices page exists (Super Admin only) for existing installations
  const { rows: kioskRows } = await pool.query(`SELECT allowed_roles FROM page_permissions WHERE page_id = 'kiosk-devices'`)
  if (kioskRows.length === 0) {
    await pool.query(
      `INSERT INTO page_permissions (page_id, page_label, allowed_roles) VALUES ('kiosk-devices', 'Kiosk Devices', '{superadmin}')`
    )
    console.log('✅ Kiosk Devices page permission added')
  } else if (!kioskRows[0].allowed_roles.includes('superadmin')) {
    await pool.query(
      `UPDATE page_permissions SET allowed_roles = ARRAY['superadmin'], updated_at = now() WHERE page_id = 'kiosk-devices'`
    )
    console.log('✅ Kiosk Devices page permission updated to superadmin only')
  }
